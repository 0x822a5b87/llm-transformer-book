...
**FFN最重要的意义在于，它为数据的线性变换加入了非线性函数，如果只有线性层（矩阵乘法），无论我们堆叠多少层 Block，整个模型在数学上依然只是一个巨大的线性回归。正是 FFN 中的激活函数（如 GELU 或 ReLU），给了神经网络“扭曲空间”的能力。**

举个例子：在线性组合下，水烧到200°时仍然是水，然而水在100°时便变成了水蒸气，这个过程并不是线性的。
