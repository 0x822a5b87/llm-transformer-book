...
Pre-Norm 的训练更稳的原因主要是：

1. `Attention` 的计算中，`softmax` 是一个指数级的函数，如果不进行norm会导致数值溢出，梯度过早饱和等问题；
2. `FFN` 通常由两层线性映射和一个激活函数（如 GELU 或 ReLU）组成，如果输入 FFN 的数值抖动很大，激活函数的输出就会在“全开”和“全关”之间剧烈跳变。
---
